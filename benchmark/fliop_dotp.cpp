#include <io/netmp.h>
#include <protocol/zkfliop_evaluator_dotp.h>
...
common::utils::Circuit<Ring> generateCircuit(size_t number, size_t dimension) {
    common::utils::Circuit<Ring> circ;

    // We let all #number many compute on the same inputs, as the input phase is anyway not included in the benchmark
    std::vector<common::utils::wire_t> a_inputs(dimension);
    std::vector<common::utils::wire_t> b_inputs(dimension);
    std::generate(a_inputs.begin(), a_inputs.end(),
                [&]() { return circ.newInputWire(); });
    std::generate(b_inputs.begin(), b_inputs.end(),
                [&]() { return circ.newInputWire(); });

    for (size_t i = 0; i < number; i++) {
        common::utils::wire_t out = circ.addGate(common::utils::GateType::kDotprod, a_inputs, b_inputs);
...
void benchmark(const bpo::variables_map& opts) {
    ...
    auto number = opts["number"].as<size_t>();
    auto dimension = opts["dimension"].as<size_t>();
    ...
    json output_data;
    output_data["details"] = {{"number", number},
                                {"dimension", dimension},
                                {"num-parties", nP},
                                {"pid", pid},
    ...
    int T = 2 * ceil(log(number * (2 * dimension + 1)) / log(compression_factor)) + 1;
    if (T > SSEC) {
    ...
    auto circ = generateCircuit(number, dimension).strictlyOrderGatesByLevel();
    std::cout << "--- Circuit ---\n";
    ...
    for (const auto& g : circ.gates_by_level[0]) {
        if (g->type == common::utils::GateType::kInp) {
            ...
            c_val++;
            c_val = c_val % dimension;
        }
    }
    

    for (size_t r = 0; r < repeat; ++r) {
        DotPEvaluator eval(nP, pid, network, circ, compression_factor, pking_semi, pking_verify);
        ...
        */
        ...
        if (pid > 0) {
            ...
            [[maybe_unused]] Ring expected = 0;
            for (size_t i = 0; i < dimension; i++) {
                expected += i * i;
            }

            for (size_t i = 0; i < number; i++) {
                assert(res[i] == expected);
            }
        }
...
bpo::options_description programOptions() {
    ...
    desc.add_options()
        ("number", bpo::value<size_t>()->required(), "Number of dot products.")
        ("dimension", bpo::value<size_t>()->required(), "Dimension of each dot product.")
