...
if "r-asterisk" in sys.argv:
    ASTERISK_PATH_SELECTOR = "_reproduced"
    print("Asterisk: using freshly benchmarked data")
else:
    ASTERISK_PATH_SELECTOR = ""
    print("Asterisk: using data from repository")
...
def get_row_impr(lan_asterisk, wan_asterisk, lan_ours, wan_ours):
    ...
    if LATEX:
        ...
        return f'{format(off_com_a / off_com_o, 1, 4)}x+{format(on_com_a / on_com_o, 1, 3)}x | {format(off_lan_a / off_lan_o, 1, 4)}x+{format(on_lan_a / on_lan_o, 1, 3)}x | {format(off_wan_a / off_wan_o, 1, 3)}x+{format(on_wan_a / on_wan_o, 1, 3)}x'
...
if __name__ == "__main__":
    ...
    for depth in DEPTH:
        with ExitStack() as stack:
            files_LAN_asterisk = [stack.enter_context(open(f"Asterisk_LAN{ASTERISK_PATH_SELECTOR}/p{p}/n{PARTIES}-c2-d{depth}-pking-0.txt", 'r')).readlines() for p in range(PARTIES + 1)]
            files_WAN_asterisk = [stack.enter_context(open(f"Asterisk_WAN{ASTERISK_PATH_SELECTOR}/p{p}/n{PARTIES}-c2-d{depth}-pking-0.txt", 'r')).readlines() for p in range(PARTIES + 1)]
            ...
            files_LAN_fliop = [stack.enter_context(open(f"LAN{PATH_SELECTOR}/p{p}/fliop-m{MULTS}-n{PARTIES}-c{COMPR}-d{depth}-pking-1-t{THREADS}.txt", 'r')).readlines() for p in range(PARTIES + 1)]
            files_WAN_fliop = [stack.enter_context(open(f"WAN{PATH_SELECTOR}/p{p}/fliop-m{MULTS}-n{PARTIES}-c{COMPR}-d{depth}-pking-1-t{THREADS}.txt", 'r')).readlines() for p in range(PARTIES + 1)]
